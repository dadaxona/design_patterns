...
<body>
    <div class="container">
        ...
        <div class="row">  
          ...
          <form action="{{ route('oopproject.store')}}" method="POST">
              @csrf
              <div class="row">
                  <div class="col-6">
            ...
            <tbody>
               ...
               @foreach ($items as $item)
                    <tr>
                        ...
                        <td>{{$item->email}}</td>       
                        <td>{{$item->password}}</td>                      
                        <td><a href="{{ route('oopproject.show',$item->id)}}"><button class="btn btn-primary">Edit</button></a></td>
                        <td>  
                            <form method="post" class="delete_form" action="{{ route('oopproject.destroy',$item->id)}}">
                                @csrf
                                    @method('DELETE')
